...
**存储桶插件管理**<br/>
存储桶管理，是由管理员进行在线开发的插件，用于管理员对存储桶插件的相关配置，需要做什么前置处理或者后置处理等进行管理，例如七牛云对象存储，需要用户在界面上感知出需要填写哪些数据、哪些数据时必填项、有哪些数据的智能提示，需要前置操作则是回去上传认证，其实就是对存储桶拥有哪些元数据进行配置，于此同时还提供了是否启用或者禁用的功能，比如某一个对象存储已经从市面上out，则管理员可以进行禁用操作，这样用户就不能创建该类型的存储桶。
...
1. 数据迁移/数据备份<br>
  ...
  - 存储桶支持一键导入所有图片(即将zip包中的图片一键批量导入到存储桶中)
2. 新增存储桶插件<br>
