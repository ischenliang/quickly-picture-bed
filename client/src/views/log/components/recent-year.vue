<template>
  <div id="recent-year" class="stats-chart"></div>
</template>
...
onMounted(() => {
  ...
  const el = document.getElementById('recent-year')
...
watch(() => props.data, (val: { [prop: string]: { month: string; count: number }[] }) => {
  if (val) {
    nextTick(() => {
      const { image, album, bucket, wiki, article } = val
        ...
        series: [
          {
            type: 'bar',
            name: '图片数量',
            data: getYaxisData(image, months)
          },
          {
            type: 'bar',
            name: '相册数量',
            data: getYaxisData(album, months)
          },
          {
            type: 'bar',
            name: '存储桶数量',
            data: getYaxisData(bucket, months)
          },
          {
            type: 'bar',
            name: '知识库数量',
            data: getYaxisData(wiki, months)
          },
          {
            type: 'bar',
            name: '文章数量',
            data: getYaxisData(article, months)
          }
        ]
      })
    })
