...
// 验证码登录
async function login_verify (params: LoginParams, header: any, ctx: Context) {
  /**
   * 1. 校验邮箱验证码
   *    1.1 首先判断验证码是否已过期
   *    1.2 其次判断内容是否匹配
  ...
  if (user) {
    ...
    await LogModel.create({
      ...
      operate_cont: params.email || params.phone,
      content: '登录成功',
      uid: user.id,
      client_info: {
        province,
...
// 密码登录
async function login_password (params: LoginParams, header: any, ctx: Context) {
  const tmp: User = {}
...
class PublicController {
  ...
  async login(@Body({ required: true }) params: LoginParams, @Header() header: any, @Ctx() ctx: Context) {
    if (params.type === 'verify_code') {
      return login_verify(params, header, ctx)
    } else {
      return login_password(params, header, ctx)
    }
  }
