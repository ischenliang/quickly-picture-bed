...
export class ImageService {
  ...
  async upload(
    ...
    ip: string,
    param: { bucket_id: number; album_id: number; image_id: number },
    uid: number,
  ) {
    ...
    // 2、构建运行沙箱
    const sandbox = await this.pluginLoaderService.resolvePlugin(
      bucket.dataValues.user_plugin.dataValues.plugin,
    );
    // 3、上传图片
    try {
      const images: FileItem[] = await this.pluginLoaderService.upload(
        sandbox,
        files,
        bucket.config,
      );
