...
export class PluginService {
  ...
  async updateIntsall(id: number, ip: string, uid: number) {
    ...
    try {
      ...
      // 3、更新安装记录
      const res = await this.userPluginModel.update(
        {
          version: plugin.version,
        },
        {
      ...
      if (user_plugin && user_plugin.id) {
        try {
          await this.sequelize.transaction(async (transaction) => {
            ...
            const userHabits = await this.habitsModel.findOne({
              where: { uid },
            });
            if (
              userHabits.current_theme &&
              userHabits.current_theme.plugin_id === plugin.id
            ) {
   ...
   * @param uid
   */
  async getUserPlugins(status: number, type: string, uid: number) {
    const data: any = {};
    const where: any = {};
    if (type) {
      where.category = type;
  ...
  async toggleIntsall(id: number, uid: number) {
    ...
    return this.userPluginModel.update(
      {
        status: userPlugin.status == 0 ? 1 : 0,
      },
      {
        where: {
          id,
