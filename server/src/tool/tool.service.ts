...
export class ToolService {
  ...
  async upload(file, path) {
    // 解决中文乱码问题
    file.originalname =
      iconv.decode(file.originalname as any, 'utf8') || file.originalname;
    // 目标目录，没有这个文件夹会自动创建
  ...
  async smsSend(param: {
    ...
    if (
      verifyCode &&
      this.timeService.diffTime(null, verifyCode.expire_at) <= 0
    ) {
      const flag =
        verifyCode.anser.toUpperCase() === param.verify_code.toUpperCase();
      if (flag) {
  ...
  async npmRegistryPackage(name: string) {
    ...
    return this.httpService.get(`https://registry.npmjs.org/${name}`).pipe(
      map((res) => {
  ...
  sendZhihuMail(
    text: string,
    to: string,
    subject: string = 'LightFastPicture',
  ) {
    console.log('发送邮件');
